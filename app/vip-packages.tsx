...
export default function VipPackages() {
  // ===== Responsive hero sizing (khắc phục chữ bị tràn) =====
  const { width } = useWindowDimensions();
  // Kích thước cơ sở theo width, kẹp trong khoảng cho chắc
  const base = Math.min(Math.max(width * 0.17, 44), 64); // px
  const sizeTop = base * 0.85;  // lớp mờ nhất
  const sizeMid = base * 1.05;  // lớp giữa
  const sizeMain = base * 1.25; // lớp đậm nhất
  const lhTop = sizeTop * 0.98;
  const lhMid = sizeMid * 0.98;
  const lhMain = sizeMain * 0.98;
  const heroHeight = sizeTop + sizeMid + sizeMain + 36; // cao hơn để không cắt
  ...
  return (
    ...
    <Stack.Screen
      ...
      <View style={styles.root}>
        {/* HERO Plans – gradient + chấm tròn + chữ xếp lớp, responsive */}
        <LinearGradient
        ...
        >
          ...
          <Text
            style={[
              styles.heroLayer,
              { fontSize: sizeTop, lineHeight: lhTop, color: '#7beb9a', opacity: 1 },
            ]}
            numberOfLines={1}
          >
            Plans
          ...
          <Text
            style={[
              styles.heroLayer,
              { fontSize: sizeMid, lineHeight: lhMid, color: '#3bd162', opacity: 0.9, marginTop: -6 },
            ]}
            numberOfLines={1}
          >
            Plans
          ...
          <Text
            style={[
              styles.heroLayer,
              { fontSize: sizeMain, lineHeight: lhMain, color: '#0e2a1a', opacity: 0.65, marginTop: -6 },
            ]}
            numberOfLines={1}
          >
            Plans
        ...
        >
          ...
          <LinearGradient colors={['#ebffd9', '#efffe7']} style={styles.card}>
            ...
            <TouchableOpacity style={styles.cta} onPress={() => onChoose(basic.plan)}>
          ...
          <LinearGradient colors={['#ebffd9', '#efffe7']} style={styles.card}>
            <View style={styles.cardRowTop}>
              ...
              <Text style={[styles.cardBadge, { backgroundColor: '#1e293b', color: '#fff' }]}>
                Đề xuất
              </Text>
            </View>
            ...
            <TouchableOpacity style={styles.cta} onPress={() => onChoose(premium.plan)}>
              <Text style={styles.ctaText}>Chọn gói này</Text>
            </TouchableOpacity>
          </LinearGradient>

          {/* Premium year (placeholder) */}
          ...
          <TouchableOpacity
            ...
            onPress={() => Linking.openURL(Linking.createURL('payments'))}
          >
